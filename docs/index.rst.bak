...
Ganja.js is a Geometric Algebra code generator for javascript. It
generates Clifford algebras and sub-algebras of any signature and
implements operator overloading and algebraic constants.
...
(**Mathematically**, an algebra generated by ganja.js is a graded
exterior (Grassmann) algebra (or one of its subalgebras) with a
non-metric outer product, extended (Clifford) with geometric and
contraction inner products, a Poincare duality operator and the main
...
with element, vector and matrix operations over **reals**, **complex
numbers**, **dual numbers**, **hyperbolic numbers**, **vectors**,
**spacetime events**, **quaternions**, **dual quaternions**,
**biquaternions** or **any other Clifford Algebra**.)

(**Seriously**, look at the
`examples <https://enkimute.github.io/ganja.js/examples/coffeeshop.html>`__,
run some quick numbers using the
`GAlculator <https://enkimute.github.io/ganja.js/examples/galculator.html>`__
or play `the wedge
game <https://enkimute.github.io/ganja.js/examples/example_game_wedge.html>`__
first.)

Discourse and Discord
---------------------

Visit `bivector.net <https://bivector.net>`__ for our forum and chat -
the perfect place for questions and support.

New !
-----

ganja.js now has a nodejs based templated source generator that allows
the creation of arbitrary algebras for **C++**, **C#**, **python** and
**rust**. The generated code provides in a flat multivector format and
operator overloading. Check the ‘codegen’ folder for the source, several
algebras are available in pregenerated versions.

-  `C++ pre-generated
   sources <https://github.com/enkimute/ganja.js/tree/master/codegen/cpp>`__
-  `C# pre-generated
   sources <https://github.com/enkimute/ganja.js/tree/master/codegen/csharp>`__
-  `rust pre-generated
   sources <https://github.com/enkimute/ganja.js/tree/master/codegen/rust>`__
-  `python pre-generated
   sources <https://github.com/enkimute/ganja.js/tree/master/codegen/python>`__

Contents
--------

`1. Reasons to use ganja <#Features>`__\  `2. Using ganja for the first
time <#Started>`__\  `3. Getting free ganja samples <#samples>`__\  `4.
Ganja for experienced users <#custom>`__\  `5. Ganja ingredients and
syntax <#syntax>`__\  `6. Ganja starterkit : PGA2D
P(R\*2,0,1) <#P2>`__\  `7. Ganja starterkit : PGA3D
P(R\*3,0,1) <#P3>`__\ 
...
-  Algebraic constants
-  Supports any metric (positive,negative,zero) and dimensionality (also
   +10)
-  smallish (20kb on the wire)
-  matrix-free inverses up to 5D.
-  geometric, inner (left contraction), outer (wedge) and regressive
   (vee) product
-  conjugate, Reverse, Involute, Dual (Poincare), Negative
-  4 API’s (inline, asciimath, object oriented, functional)
-  Easy graph function for 1D and 2D functions, Projective 2D, 3D and
   conformal 2D and 3D elements. (SVG/webGL/OPNS)
-  Supports vectors and matrices in all its algebras.
-  There’s a
   `game <https://enkimute.github.io/ganja.js/examples/example_game_wedge.html>`__
   that teaches you how to use ganja.js !
...
Or in the browser, just include the ganja.js script. (ganja.js has no
dependencies)

.. figure:: https://img.shields.io/npm/v/ganja.js
   :alt: npm
...
To create an Algebra, call the **Algebra** function specifying the
metric signature (number of positive,negative and zero dimensions). The
result is an ES6 class implementing the requested clifford algebra.
...
An extended syntax is also available that allows you to further tweak
the created Algebra.
...
You can now use these classes to generate algebraic elements. Those
elements will have all of the expected properties. (Length, blade
access, Dot, Wedge, Mul, Dual, Inverse, etc …)

And while not advised you could use them in a ‘classic’ programming
style syntax like the example below.
...
This however, is not very pretty. It’s not that much fun either.
Luckily, ganja.js provides an alternate way to write algebraic
functions, literals and expressions.

The inline function
~~~~~~~~~~~~~~~~~~~

Your Algebra class exposes this interface through the **inline**
function. It accepts a javascript function, and translates it to use the
Algebra of your choice. Using the **inline** function, the above example
is written :
...
Note that if you are immediately executing the function, you can add it
as a last parameter to your Algebra constructor call.
...
specify basis blades and allows using arrays or lambda expressions
without the need for calling brackets in algebraic expressions.
...
In the example above, functions **a** and **b** do the same thing, but
it should be clear that **a-b=headeache**. Because I’m out of aspirin,
I’ll leave the proof of that to the reader.

See the
`coffeeshop <https://enkimute.github.io/ganja.js/examples/coffeeshop.html>`__
for more examples of how to use the inline syntax.

The graph function.
~~~~~~~~~~~~~~~~~~~

Your Algebra also exposes a static **graph** function that allows you to
easily graph 1D or 2D functions as well as 2D and 3D PGA and CGA
elements.
...
Again, many more examples can be found at `the
coffeeshop <https://enkimute.github.io/ganja.js/examples/coffeeshop.html>`__.
...
Please visit `the
coffeeshop <https://enkimute.github.io/ganja.js/examples/coffeeshop.html>`__
and play around with the examples. They are interactive and you can
easily change the code online. No need to download or install anything !
...
ganja.js is also the engine behind the GAlculator - try it
`online <https://enkimute.github.io/ganja.js/examples/galculator.html>`__
or get it on the `play
store <https://play.google.com/store/apps/details?id=io.github.enkimute>`__

Or - get some hands on experience with euclidian plane PGA by playing
the `wedge
game <https://enkimute.github.io/ganja.js/examples/example_game_wedge.html>`__.

## Ganja for experienced users.

Ganja.js allows you to further customise the algebra class it generates,
allowing you to generate subalgebras (who’s elements use less storage),
or algebra’s where you decide on the order and name of the basis blades.
(the name should always be exyz but you can pick e.g. e20 instead of the
default e02 and expect ganja.js to make appropriate sign changes)
...
When not specified, ganja.js will generate basis names that are grouped
by rank and numerically sorted. By default, a single zero dimension will
get generator name e0. Zero dimensions come first.
...
+-----------------------------------+-----------------------------------+
| 4,0,0 and 3,1,0                   | 1,e1,e2,e3,e4,e12,e13,e14,e23,    |
|                                   | e24,e34,e123,e124,e134,e234,e1234 |
+-----------------------------------+-----------------------------------+
| 3,0,1                             | 1,e0,e1,e2,e3,e01,e02,e03,e12,    |
|                                   | e13,e23,e012,e013,e023,e123,e0123 |
+-----------------------------------+-----------------------------------+

*note* the scalar part of a multivector **“mv”** can be addressed with
**“mv.s”**, other basis blades follow the expected pattern.
e.g. **“mv.e12”** or **“mv.e012”**.

By default, your algebra elements will inherit from Float32Array. You
can change the underlying datatype used by ganja.js to any of the typed
array basis types :
...
Or take things a bit further and simply specify a Cayley table to your
liking. The example below shows automatic numerical differentiation and
calculates the value, 1st, 2nd and 3rd derivative of any polynomial.
...
For storage and performance reasons it can be interesting to combine
elements of various sub-algebras of a given Algebra. Ganja.js supports
this by setting options.mix to true when you create your algebra.
...
With the **mix** mode enabled, all operations generated by ganja.js will
use basis name access instead of array indexing. (and all operations are
protected to substitute missing blades with 0).

The Inline syntax can still be used, keep in mind that in most cases you
would want that to be the inline function of the ‘parent’ algebra. (In
the example above, use R2.Inline and not C.Inline as the latter will
reduce all your operations to the field of C).
...
for Wedge, Vee, Dot and Sandwich which have higher precedence than \*
and /, resulting in less brackets in many common GA expressions.

+-----------+----------+-----------+-------------------+-------------+
| P         | Inline   | AsciiMath | Object Oriented   | Functional  |
| recedence | JS       |           |                   |             |
+===========+==========+===========+===================+=============+
| 5         | x.       | tilde(x)  | x.Involute        | A.          |
|           | Involute |           |                   | Involute(x) |
+-----------+----------+-----------+-------------------+-------------+
| 5         | x        | ddot(x)   | x.Reverse         | A           |
|           | .Reverse |           |                   | .Reverse(x) |
+-----------+----------+-----------+-------------------+-------------+
| 5         | ~x       | hat(x)    | x.Conjugate       | A.C         |
|           |          |           |                   | onjugate(x) |
+-----------+----------+-----------+-------------------+-------------+
| 5         | !x       | bar(x)    | x.Dual            | A.Dual(x)   |
+-----------+----------+-----------+-------------------+-------------+
| 4 rtl     | x**-1    | x^-1      | x.Inverse         | A           |
|           |          |           |                   | .Inverse(x) |
+-----------+----------+-----------+-------------------+-------------+
| 4 rtl     | x**y     | x^y       | x.Pow(y)          | A.Pow(x,y)  |
+-----------+----------+-----------+-------------------+-------------+
| 3         | x^y      | x^^y      | x.Wedge(y)        | A           |
|           |          |           |                   | .Wedge(x,y) |
+-----------+----------+-----------+-------------------+-------------+
| 3         | x<<y     | x*y       | x.Dot(y)          | A.Dot(x,y)  |
+-----------+----------+-----------+-------------------+-------------+
| 3         | x&y      | b         | x.Vee(y)          | A.Vee(x,y)  |
|           |          | ar(bar(x) |                   |             |
|           |          | ^^bar(y)) |                   |             |
+-----------+----------+-----------+-------------------+-------------+
| 3         | x>>>y    | x \*\* y  | x.Mul(y)          | A.sw(x,y)   |
|           |          | \*\*      | .Mul(x.Conjugate) |             |
|           |          | hat(x)    |                   |             |
+-----------+----------+-----------+-------------------+-------------+
...
+-----------+----------+-----------+-------------------+-------------+
|           | 2e2      | 2e_2      | new A([0,0,2,0])  | A.          |
|           |          |           |                   | Vector(0,2) |
+-----------+----------+-----------+-------------------+-------------+
|           | 2e12     | 2e_12     | new A([0,0,0,2])  | A.          |
|           |          |           |                   | Bivector(2) |
+-----------+----------+-----------+-------------------+-------------+
...
vectors that are not in **x**. For non-degenerate metrics, you can still
use multiplication with the pseudoscalar if so desired (although it will
be less efficient)
...
as the dot product between **x** and **y** gives the orthogonal
complement in **y** of the projection of **x** onto **y**.

Vee product
~~~~~~~~~~~

The vee product is available as an optimized shorthand for the dual of
...
I’ve chosen the **&** symbol as it can be interpreted as ‘join’ or
‘meet’ depending on the geometric meaning given to vectors.
(planes/lines or points)
...
effectively making this P(R\*2,0,1).

Simply include the ganja.js script and put the block below in a
**SCRIPT** tag to get started ..
...
.. figure:: images/ganja_p2.jpg
   :alt: ganja p2 example

   ganja p2 example

## Ganja starterkit : PGA3D P(R\*3,0,1)

This example implements the table on page 15 of `Gunn’s Geometric
Algebra for Computer
Graphics <http://page.math.tu-berlin.de/~gunn/Documents/Papers/GAforCGTRaw.pdf>`__.
We apply the same strategy from above and start from a Clifford Algebra
in R3,0,1.
...
.. figure:: images/ganja_p2.jpg
   :alt: ganja p3 example

   ganja p3 example

.. |DOI| image:: https://zenodo.org/badge/98999731.svg
   :target: https://zenodo.org/badge/latestdoi/98999731
